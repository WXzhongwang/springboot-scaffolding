 ...
 * @version V1.0.0
 * @description ThreadPoolExecutor线程池 + Queue缓冲队列实现高并发中进行下单业务
...
public class ThreadPoolManager implements BeanFactoryAware {
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            ...
            System.out.println("系统任务太忙了,把此订单交给(调度线程池)逐一处理，" +
                    "订单号：" + ((BusinessThread) r).getAcceptStr());
        }
    };
    ...
    final ScheduledFuture scheduledFuture = scheduler.scheduleAtFixedRate(new Runnable() {
        ...
        public void run() {
            //判断缓冲队列是否存在记录
            if(!msgQueue.isEmpty()){
                //当线程池的队列容量少于WORK_QUEUE_SIZE，则开始把缓冲队列的订单加入到线程池
